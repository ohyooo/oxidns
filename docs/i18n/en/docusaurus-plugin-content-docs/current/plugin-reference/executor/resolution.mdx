...
- Missing, empty, or invalid rules cause plugin initialization to fail.

### Behavior

- Checks `args` against the unmodified transport peer and ignores ECS when the
  peer is not trusted.
- Replaces the client IP when the source is trusted, ECS exists, and its source prefix is non-zero,
