...
```yaml
- exec: client_ip_from_ecs
```

### Behavior

- Replaces the client IP when ECS exists and its source prefix is non-zero,
  while preserving the original connection port.
- Makes no change when ECS is absent or its source prefix is `0`.
...
Requesters can forge ECS. Enable this only when OxiDNS accepts queries solely
from a trusted dnsmasq, and configure dnsmasq with `--strip-subnet` plus
